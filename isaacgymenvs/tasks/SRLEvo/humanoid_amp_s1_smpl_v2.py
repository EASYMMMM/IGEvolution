...
from isaacgymenvs.tasks.SRLEvo.humanoid_amp_s1_smpl_base_v2 import HumanoidAMP_s1_Smpl_Base_v2, dof_to_obs
...
NUM_AMP_OBS_PER_STEP = 13 + 52 + 28 + 12 # [root_h, root_rot, root_vel, root_ang_vel, conceptual_joint_6d_obs, raw_hinge_dof_vel, key_body_pos]


class HumanoidAMP_s1_Smpl_v2(HumanoidAMP_s1_Smpl_Base_v2):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self._state_init = HumanoidAMP_s1_Smpl_v2.StateInit[state_init]
        self._hybrid_init_prob = cfg["env"]["hybridInitProb"]
        self._num_amp_obs_steps = cfg["env"]["numAMPObsSteps"]
        assert(self._num_amp_obs_steps >= 2)
        self._num_state_hist_steps = cfg["env"].get("numStateHistSteps", 5)
        self._humanoid_dof_mode = cfg["env"].get("humanoidDofMode", "legacy_expmap")
    ...
    def _load_motion(self, motion_file):
        ...
        print(self._key_body_ids.cpu().numpy())
        self._motion_lib = MotionLibV2(motion_file=motion_file,
                                       num_dofs=self.num_dof,
                                       key_body_ids=self._key_body_ids.cpu().numpy(),
                                       device=self.device,
                                       dof_representation=self._humanoid_dof_mode)
        return
    ...
    def reset_idx(self, env_ids):
        ...
        self._print_pre_physics_reset_diagnostics(env_ids)
    ...
    def _print_pre_physics_reset_diagnostics(self, env_ids):
        if not self._diag_print_pre_physics_reset:
            return
        ...
        if len(self._reset_default_env_ids) > 0 and torch.any(self._reset_default_env_ids == env_id):
            reset_source = "default"
        elif len(self._reset_ref_env_ids) > 0 and torch.any(self._reset_ref_env_ids == env_id):
            reset_source = "reference"

        root_state = self._root_states[env_id].detach().cpu().numpy()
        dof_pos = self._dof_pos[env_id].detach().cpu().numpy()
        dof_vel = self._dof_vel[env_id].detach().cpu().numpy()
        body_pos = self._rigid_body_pos[env_id].detach()
        body_heights = body_pos[:, 2]
        contact_norm = torch.norm(self._contact_forces[env_id].detach(), dim=-1)
        lowest_body_ids = torch.argsort(body_heights)[:min(12, self.num_bodies)].cpu().tolist()
        active_contact_ids = torch.nonzero(contact_norm > 0.1, as_tuple=False).flatten().cpu().tolist()

        print("=" * 120)
        print(f"[V2 Pre-Physics Reset Diagnostics] env={env_id} reset_source={reset_source}")
        print(
            "This snapshot is after reset tensors are written and before the next simulate() call. "
            "Contact forces may still be cached from the previous solver step."
        )
        print(
            f"root_pos={np.array2string(root_state[0:3], precision=5)} "
            f"root_rot={np.array2string(root_state[3:7], precision=5)}"
        )
        print(
            f"root_lin_vel={np.array2string(root_state[7:10], precision=5)} "
            f"root_ang_vel={np.array2string(root_state[10:13], precision=5)}"
        )
        system_com_pos, system_com_vel, total_mass = self._compute_system_com_state(env_id)
        print(
            f"system_com_pos={np.array2string(system_com_pos.detach().cpu().numpy(), precision=5)} "
            f"system_com_vel={np.array2string(system_com_vel.detach().cpu().numpy(), precision=5)} "
            f"total_mass={float(total_mass):.6f}"
        )
        print(f"dof_pos={np.array2string(dof_pos, precision=5, suppress_small=True, max_line_width=240)}")
        print(f"dof_vel={np.array2string(dof_vel, precision=5, suppress_small=True, max_line_width=240)}")
        print(
            f"dof_abs_max: pos={float(np.max(np.abs(dof_pos))):.6f} "
            f"vel={float(np.max(np.abs(dof_vel))):.6f}"
        )
        print("lowest body origins (body origin/COM height, not collision-geometry bottom):")
        for body_id in lowest_body_ids:
            pos = body_pos[body_id].cpu().numpy()
            print(
                f"  {body_id:02d} {self._body_names[body_id]}: "
                f"pos={np.array2string(pos, precision=5)} "
                f"contact_norm={float(contact_norm[body_id]):.5f}"
            )
        print("cached_contacts=" + str([
            f"{self._body_names[i]}({float(contact_norm[i]):.5f})" for i in active_contact_ids
        ]))
        print("=" * 120)
        return
    ...
    def _reset_actors(self, env_ids):
        if (self._state_init == HumanoidAMP_s1_Smpl_v2.StateInit.Default):
            self._reset_default(env_ids)
        elif (self._state_init == HumanoidAMP_s1_Smpl_v2.StateInit.Start
              or self._state_init == HumanoidAMP_s1_Smpl_v2.StateInit.Random):
            self._reset_ref_state_init(env_ids)
        elif (self._state_init == HumanoidAMP_s1_Smpl_v2.StateInit.Hybrid):
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        if (self._state_init == HumanoidAMP_s1_Smpl_v2.StateInit.Random
            or self._state_init == HumanoidAMP_s1_Smpl_v2.StateInit.Hybrid):
